...
* **D0c (per-p split summed over primes)**: `tsum_neg_log_eq_layers`,
...
Bridges consumed by the later Step D tracks (D1–D4):
...
/-- D0c: summability of the `layerA` integrand for `Re s > 1`. -/
lemma summable_layerA_term {s : ℂ} (hs : 1 < s.re) :
    Summable (fun p : Nat.Primes ↦ χ ((p : ℕ) : ZMod 4) * ((p : ℕ) : ℂ) ^ (-s)) :=
  (summable_norm_term_one hs).of_norm

/-! ## D0c: the per-p split summed over primes -/

/-- **D0c (per-p split summed over primes)**: for `Re s > 1`,
...
/-- D0c bridge: the ℕ-indexed coefficient function of `layerA` — `raceKernel n` at
...
/-- D0c bridge: the partial sums of `fChi` are exactly the race sums,
...
/-- D0c bridge: `layerA` as a ℕ-indexed Dirichlet series,
...
/-! ## Real companion on the real axis (consumed by D4-side tracks) -/

/-- D0c real companion of `layerA` (real `rpow`; agreement: `layerA_ofReal`). -/
noncomputable def layerAReal (σ : ℝ) : ℝ :=
  ∑' p : Nat.Primes, ((raceKernel (p : ℕ) : ℤ) : ℝ) * ((p : ℕ) : ℝ) ^ (-σ)

/-- D0c: for real `σ`, `layerA` is the cast of `layerAReal` (unconditional). -/
