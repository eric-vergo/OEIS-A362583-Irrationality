...
/-!
# Case `c = 0` endgame: the race sum is never linear (PROOF.md Step D, D1–D4)
...
* **(D3)** By the identity theorem on the open preconnected `Ω` (the `Audit/M08.lean`
  skeleton), `exp ∘ contLog = L(χ, ·)` on all of `Ω` (`exp_contLog_eqOn`).
* **(D4, single-point form)** `L(χ, ·)` is entire, hence continuous at `1/2` with some
  ...
  No filters toward `1/2⁺` are used (route decision 4).
-/
...
/-- D2: the continued logarithm `G := Ã + B + T` (PROOF.md D2), where `Ã = bpSeries fChi`
...
continuity of the entire `L(χ, ·)` at `1/2` — evaluated at a single point `σ*`, no
filters (D4, route decision 4). -/
