...
* D0c preparation: the per-prime split `neg_log_split`, the `layerB` identification
  `tsum_term_two_eq_layerB`, and the three absolute-summability facts
...
(D0c; see `tsum_term_two_eq_layerB`). -/
...
/-- `2^(-1/2) ≤ 3/4`: the crude constant behind the geometric-tail bound (D0b). -/
...
/-- `‖χ(p) p^(-s)‖ ≤ 3/4` on `Re s ≥ 1/2` (D0b geometric-tail bound). -/
...
/-- `‖χ(p) p^(-s)‖ ≤ 1/2` on `Re s ≥ 1` (domain of the per-prime split, D0c). -/
...
/-- `‖χ(p) p^(-s)‖ ≤ p^(-1/2)` on `Re s ≥ 1/2` (feeds the `p^(-3/2)` bound, D0b). -/
...
/-- D0b: per-prime `k ≥ 3` tail `t_p(s) = Σ_{k≥0} (χ(p) p^(-s))^(k+3) / (k+3)`.
...
/-- D0b: the `k ≥ 3` layer `T(s) = Σ_p t_p(s)`. -/
noncomputable def layerT (s : ℂ) : ℂ := ∑' p : Nat.Primes, tp p s

/-- D0a: the `k = 2` layer `B(s) = (1/2) Σ_{p odd} p^(-2s)`, with an `if`-mask at
...
/-- R1 step 1': `Σ 1/p` over odd primes is still non-summable (drop the `p = 2` term). -/
...
/-- R1 step 2 (continuity padding): a finite rpow sum exceeding `M` at `u₀` still
...
/-- D0a: summability of the real `layerB` integrand for `σ > 1/2`. -/
...
/-! ## Summability and bounds for the layers (D0a, D0b) -/

/-- D0a: absolute summability of the complex `layerB` integrand for `Re s > 1/2`. -/
...
/-- D0c fact 3: absolute summability of `‖t_p(s)‖` over primes on `Re s ≥ 1/2`. -/
...
/-- D0b: plain summability of the `layerT` integrand on `Re s ≥ 1/2`. -/
lemma summable_tp {s : ℂ} (hs : 1 / 2 ≤ s.re) : Summable (fun p : Nat.Primes ↦ tp p s) :=
  (summable_norm_tp hs).of_norm

/-- **D0b uniform bound**: `‖T(s)‖ ≤ C_T` on `Re s ≥ 1/2`. -/
...
/-- D0a: `0 ≤ layerBReal σ` (unconditional: a non-summable `tsum` is `0`). -/
...
/-- **D0a bound**: `layerBReal σ ≤ C_B` for real `σ ≥ 1`. -/
...
/-- D0b: for real `σ`, `tp` is the cast of `tpReal` (unconditional). -/
...
/-- D0b: real-companion bound `|layerTReal σ| ≤ C_T` for real `σ ≥ 1/2`. -/
...
/-! ## Holomorphy on Ω = {Re s > 1/2} (D0a, D0b) -/

/-- D0b: each `t_p` is holomorphic on `Ω` (M-test with the uniform geometric bound
...
/-- **D0b holomorphy**: `T` is holomorphic on `Ω = {Re s > 1/2}` (Weierstrass M-test
...
/-- D0a auxiliary: `layerB` is holomorphic on each `{Re s > δ}`, `δ > 1/2` (M-test with
...
/-- **D0a holomorphy**: `B` is holomorphic on `Ω = {Re s > 1/2}`.  No single summable
...
/-- D0c fact 1: absolute summability of the `k = 1` terms for `Re s > 1`
...
/-- D0c fact 2: absolute summability of the `k = 2` terms for `Re s > 1/2`
