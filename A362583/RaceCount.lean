...
/-!
# A362583: eventually periodic bits give a linear race sum

If the bit sequence `bit` is eventually periodic (period `P`, from index `N₀`), then the
Chebyshev race sum is linear in the prime count: `raceSum N = c·π(N) + O(1)` with
`c = (P - 2j)/P` (here realized as `c = W/P`, where `W = ∑_{k∈[N₀,N₀+P)} (1 - 2·bit k)` is
the exact window sum).

The argument has three parts:

* the bridge `raceSum_bridge`: `raceSum N = ∑_{k < π(N) - 1} (1 - 2·bit k)`, the filtered
  χ₄-sum over primes `≤ N` reindexed by odd-prime index, by induction on `N` with
  `Nat.nth_count` supplying the enumeration step;
* periodic counting (`exists_c_bound`): an eventually periodic ±1 sequence has partial sums
  `c·m + O(1)`, with the crude explicit bound `2·N₀ + 2·P`;
* assembly (`raceSum_linear_of_eventuallyPeriodic`): absorb the `m = π(N) - 1` reindexing
  shift (cost `|c| ≤ 1`) into the constant `C = 2·N₀ + 2·P + 1`.
-/

namespace A362583

/-! ### The χ₄ summand reindexed by odd-prime index -/

/-- χ₄ at the `k`-th odd prime through the bit sequence: `1 - 2·bit k ∈ {±1}`. -/
private noncomputable def term (k : ℕ) : ℤ := 1 - 2 * (bit k : ℤ)

/-- The summand `term k = 1 - 2·bit k` is a `±1` value, so `|term k| ≤ 1`. -/
private lemma abs_term_le_one (k : ℕ) : |term k| ≤ 1 := by
  unfold term bit
  split <;> norm_num

/-- Each `oddPrime k` is prime — it is a value of `Nat.nth Nat.Prime`. -/
...
/-- The `raceKernel` value at the `k`-th odd prime is `1 - 2·bit k`. -/
private lemma raceKernel_oddPrime (k : ℕ) : raceKernel (oddPrime k) = term k := by
  unfold raceKernel term bit
  rcases oddPrime_mod_four k with h | h <;> rw [h] <;> norm_num

/-! ### Stepping `raceSum` and `Nat.primeCounting` -/

/-- `raceSum` as an unfiltered range sum, `∑_{p ≤ N} (if p prime then χ₄(p) else 0)` — the
form that steps under `Finset.sum_range_succ`. -/
private lemma raceSum_eq_range_sum (N : ℕ) :
    raceSum N = ∑ p ∈ Finset.range (N + 1), if Nat.Prime p then raceKernel p else 0 := by
  rw [raceSum_eq_sum_raceKernel, Finset.sum_filter]

/-- Stepping the race sum past a prime: `raceSum (n+1) = raceSum n + χ₄(n+1)`. -/
private lemma raceSum_succ_of_prime {n : ℕ} (hp : Nat.Prime (n + 1)) :
    raceSum (n + 1) = raceSum n + raceKernel (n + 1) := by
  rw [raceSum_eq_range_sum (n + 1), raceSum_eq_range_sum n, Finset.sum_range_succ, if_pos hp]

/-- Stepping the race sum past a non-prime leaves it fixed: `raceSum (n+1) = raceSum n`. -/
...
/-- The prime count increments at a prime: `π(n+1) = π(n) + 1`. -/
...
/-- The prime count is fixed at a non-prime: `π(n+1) = π(n)`. -/
...
/-- **The bridge**: the race sum over primes `≤ N` equals the sum of
  ...
  | succ n ih =>
      ...
      · by_cases h2 : n = 1
        ...
        · -- n + 1 is an odd prime: both sides gain the same term
          ...
          rw [← hodd, raceKernel_oddPrime]
      · rw [raceSum_succ_of_not_prime hp, primeCounting_succ_of_not_prime hp, ih]

/-! ### Partial sums of an eventually periodic ±1 sequence are linear + O(1) -/

/-- Any `n` consecutive `term`s sum to at most `n` in absolute value. -/
...
/-- Shifting a full window by whole periods does not change its sum. -/
...
/-- `q` complete periods contribute exactly `q` window sums. -/
...
/-- **Long regime** (`N₀ ≤ m`): write `m = N₀ + q·P + r` with `r < P` and split
...
/-- **Periodic counting**: with `c = W/P` (window sum over period), the `term`-partial sums
track `c·m` within the generous constant `2·N₀ + 2·P`.  Only a weakest-sufficient bound is
needed, so the two regimes `m ≤ N₀` and `N₀ ≤ m` are dispatched to the crude block bounds
...
/-- An eventually periodic bit sequence makes
