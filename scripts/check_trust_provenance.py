...
(codex-audit CX-078). Nothing below is taken from the record on faith:
  ...
  * each of the four comparator roles must appear EXACTLY ONCE and name the
    canonical path this consumer configured, parsed here out of
    `site/lakefile.lean` rather than restated;
...
Run: python3 scripts/check_trust_provenance.py --help
"""
...
def parse_configured_paths(lakefile_text, v):
...
def check_lake_input_edges(lakefile_text, configured, v):
...
def check(record, lakefile_text, build_revision, deployable, event, root):
    v = Violations()
    configured = parse_configured_paths(lakefile_text, v)
    check_lake_input_edges(lakefile_text, configured, v)
...
def build_parser():
    ...
    p.add_argument("--lakefile", required=True, help="site/lakefile.lean, the configuration of record")
    p.add_argument(
        "--build-revision",
        required=True,
    ...
    p.add_argument(
        ...
        required=True,
        choices=("true", "false"),
        help="whether this run's artifact can reach Pages; must be the SAME expression as deploy.if",
    )
