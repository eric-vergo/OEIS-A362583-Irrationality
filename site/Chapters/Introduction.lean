...
to $`1`. Reading the digits as a binary expansion produces the *Rokicki's constant* $`\varrho`
$$`\varrho \;=\; 0.10110011\ldots_2 \;=\; \sum_{k \ge 0} b_k\, 2^{-(k+1)} \;\approx\; 0.7004,`


Chebyshev observed in 1853 that primes $`\equiv 3 \pmod 4` appear to lead primes $`\equiv 1 \pmod 4`
...
The classical irrationality routes for constants of this kind run through Littlewood's 1914
$`\Omega_\pm`-oscillation for the race, or through long runs of consecutive primes in a fixed
classes. Neither is needed: refuting the linear trajectory requires only *non-degeneracy* of the race,
not quantitative oscillation. The analytic inputs are exactly five, all present in Mathlib: the analytically continued
