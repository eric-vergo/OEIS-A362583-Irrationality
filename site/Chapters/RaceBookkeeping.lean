...
:::group "race"
Step C: eventually periodic bits make the Chebyshev race sum linear in the prime count, up to
a bounded error.
:::

This chapter is pure bookkeeping — no analysis, no characters. Its single theorem converts
the digit-level conclusion of Step B into the statement that Step D will refute: a linear
race $`S(N) = c\,\pi(N) + O(1)`.

:::theorem "thm:race-linear" (lean := "A362583.raceSum_linear_of_eventuallyPeriodic") (parent := "race") (uses := "def:raceSum, def:bit")
*Step C.* If the bit sequence is eventually periodic — $`b_{k+P} = b_k` for all
...
the $`n`-th-prime function and the prime-counting function — the bookkeeping burden this
chapter exists to pay.)
